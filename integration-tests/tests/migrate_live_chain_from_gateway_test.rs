...
async fn run_migrate_live_chain_from_gateway_test() -> Result<()> {
    ...
    // Stop the pre-migration chain server before waiting for drain. Otherwise
    // its idle batch sealing can keep `totalBatchesCommitted` one batch ahead
    // of `totalBatchesExecuted` forever, racing the Migrator's
    // `NotAllBatchesExecuted` check.
    chain_server
        .stop()
        .context("stop gateway-settling chain server before migrate-from submit")?;
    ...
    // gateway-side diamond, so wait for the gateway to drain those batches
